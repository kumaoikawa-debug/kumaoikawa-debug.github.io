...
const sp1 = photoSet(fakeXf, ["scenic", "people", "action", "detail", "cover", "team"], 6, "c.jpg");
chk("photoSet-排除封面后不含封面", sp1.every((p) => p.src !== "c.jpg") && sp1.length === 1, "len=" + sp1.length);
chk("photoSet-排除封面后正文无重复", sp1.length === new Set(sp1.map((p) => p.src)).size);
...
const sp0 = photoSet(onlyCover, ["scenic", "people", "action", "detail", "cover", "team"], 6, "c.jpg");
chk("photoSet-仅封面时正文为空", sp0.length === 0, "len=" + sp0.length);
...
const spm = photoSet(manyXf, ["scenic"], 20);
chk("photoSet-多图无内部重复", spm.length === 10 && spm.length === new Set(spm.map((p) => p.src)).size, "len=" + spm.length);
...
if (typeof attachPhotoCaptions === "function") xf.master.keyImages = await attachPhotoCaptions(xf.master.keyImages, xf.master.confirmedFacts, strat.editorialDirection);
const cover = gzhCover(xf);
const secPhotos = photoSet(xf, ["scenic", "people", "action", "detail", "cover", "team"], 6, cover.src);
...
// --- 3) 降级档位 ---
chk("降级-少图档(<=3)", photoDowngrade({ photos: ["a", "b"] }).tier === "few");
chk("降级-偏少档(4-8)", photoDowngrade({ photos: ["a", "b", "c", "d", "e"] }).tier === "mild");
chk("降级-充足档(>=9)", photoDowngrade({ photos: Array.from({ length: 10 }, (_, i) => "a" + i) }).tier === "full");
chk("降级-少图提示非空", !!photoDowngrade({ photos: ["a", "b"] }).hint);
chk("降级-充足提示为空", photoDowngrade({ photos: Array.from({ length: 10 }, (_, i) => "a" + i) }).hint === "");

// --- 4) 结果页提示 ---
const sb = styleBar({ scenario: "recruit", family: "route_editorial", variant: 0, strategy: { editorialDirection: { angle: "x" } }, photos: ["a", "b"], photoOverrides: { excluded: {} } });
chk("结果页-少图降级提示可见", sb.indexOf("克制留白版式") >= 0);
const sbFull = styleBar({ scenario: "recruit", family: "route_editorial", variant: 0, strategy: { editorialDirection: { angle: "x" } }, photos: Array.from({ length: 10 }, (_, i) => "a" + i), photoOverrides: { excluded: {} } });
