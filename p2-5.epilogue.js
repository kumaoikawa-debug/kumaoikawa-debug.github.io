...
const pq0 = xfPageQuality(xf0, goodOut);
...
const fsClean = xfPageQuality(mkXf(baseMaster, cleanOut), cleanOut).dims.factSafety;
const fsFict = xfPageQuality(mkXf(baseMaster, fictOut), fictOut).dims.factSafety;
...
const rpUniq = xfPageQuality(mkXf(baseMaster, uniqOut), uniqOut).dims.repeat;
const rpDup = xfPageQuality(mkXf(baseMaster, dupOut), dupOut).dims.repeat;
...
const thOn = xfPageQuality(mkXf(baseMaster, onTheme), onTheme).dims.theme;
const thOff = xfPageQuality(mkXf(baseMaster, offTheme), offTheme).dims.theme;
...
const imGood = xfPageQuality(mkXf(imgGoodMaster, sceneText), sceneText).dims.image;
const imPoor = xfPageQuality(mkXf(imgPoorMaster, sceneText), sceneText).dims.image;
...
const tpPlain = xfPageQuality(mkXf(baseMaster, plainOut), plainOut).dims.template;
const tpTpl = xfPageQuality(mkXf(baseMaster, tplOut), tplOut).dims.template;
...
const emVivid = xfPageQuality(mkXf(baseMaster, vividOut), vividOut).dims.emotion;
const emFlat = xfPageQuality(mkXf(baseMaster, flatOut), flatOut).dims.emotion;
chk("情绪感染力-感官互动更高", emVivid > emFlat, "vivid=" + emVivid + " flat=" + emFlat);

// --- 8) 结果页不暴露数字（仅定性档位） ---
const sb = xfStyleBar({ scenario: "recruit", family: "route_editorial", variant: 0, strategy: { editorialDirection: { angle: "x" } }, photos: ["s.jpg", "w.jpg"], photoOverrides: { excluded: {} }, pageQuality: pq0 });
...
let robust = true;
try { const r1 = xfPageQuality({}, null); const r2 = xfPageQuality({ master: {} }, null); robust = r1 && r2 && typeof r1.overall === "number"; } catch (e) { robust = false; }
chk("鲁棒性-缺参不崩溃", robust);

// --- 10) 生成链路挂载：xfQualityCheck 后 state.xf.pageQuality 存在 ---
let mounted = false;
try {
  ...
  state.xf = xfGen;
  xfQualityCheck(goodOut, { family: "route_editorial", variant: 0 }, "recruit", baseMaster.confirmedFacts);
