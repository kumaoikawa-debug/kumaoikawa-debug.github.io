...
/* 当前品牌档案：从 /api/content/brand-profile 取到后调用 setBrandProfile 注入。
   ★ v223：之前只定义了 setBrandProfile 却全项目无人调用 —— 本地 director 永远走中性默认，
     老板也没有任何入口配置档案。现在由 ensureBrandProfile() 在生成前主动拉取。 */
...
/* 拉取一次品牌档案（幂等：已拉过直接用缓存，并发共用同一个 in-flight 请求）。
   拿不到（无后端 / 未登录 / 未设置）一律保持 null → brandBrief 回落中性默认，绝不阻塞生成。 */
...
async function aiContentDirector(a) {
  ...
  /* v223：prompt 拼接前先取品牌档案（拿不到就是中性默认，不阻塞生成） */
