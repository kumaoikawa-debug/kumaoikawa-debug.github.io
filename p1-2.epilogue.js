// P1-2 图片使用前轻确认验收：
// ① 面板含「上传N→建议M」统计 + 改封面(xfSetCover)/删除某图(xfToggleExclude)/使用AI推荐(xfUseRecommended)
...
const review = xfPhotoReview(xf0);
chk("轻确认面板-存在", review.indexOf("智能筛图结果") >= 0);
chk("轻确认面板-含建议统计(建议使用)", /建议使用/.test(review));
chk("轻确认面板-改封面(xfSetCover)", review.indexOf('data-action="xfSetCover"') >= 0);
chk("轻确认面板-删除某图(xfToggleExclude)", review.indexOf('data-action="xfToggleExclude"') >= 0);
chk("轻确认面板-使用AI推荐(xfUseRecommended,改动后出现)", xfPhotoReview({ scenario: "recruit", photos, photoOverrides: { cover: 2, excluded: {} }, _a: a }).indexOf('data-action="xfUseRecommended"') >= 0);
chk("轻确认面板-未改动时不显示重置", review.indexOf('data-action="xfUseRecommended"') < 0);

// --- 2) 删除某图：从生成图池剔除 ---
const xf1 = { scenario: "recruit", photos, photoOverrides: { cover: null, excluded: { "p1.jpg": true } }, _a: a };
const active1 = xfActivePhotos(xf1);
...
state.xf = xf2; // genStrategy 内部 xfPickFamily 等会读 state.xf
const strat = await genStrategy(a, xfActivePhotos(xf2), "", "recruit");
xf2.strategy = strat;
xfApplyCoverOverride(xf2);
...
state.xf = xf3;
const strat3 = await genStrategy(a, xfActivePhotos(xf3), "", "recruit");
xf3.strategy = strat3;
xfApplyCoverOverride(xf3);
