...
//   6) 质量检查 xfContentQuality：未确认实际人数时把"XX人参加"判为无依据；已确认时放行
...
var adS = xfActualActivityData(aStruct, "实际到场 20 人"); // notes 与结构化冲突，结构化应胜出
...
var adR = xfActualActivityData(aReg, "");
...
var dir = mkDir();
var type = xfRecapType(aReg, photos5);
var outR = xfFallbackRecap(aReg, mR, dir, photos5, "", type, xfActualActivityData(aReg, ""));
var tR = allText(outR);
...
var qNull = xfContentQuality(fakeNull, dir, "recap", mR.confirmedFacts, xfActualActivityData(aReg, ""));
...
var outC = xfFallbackRecap(aConf, mC, dir, photos5, "", type, xfActualActivityData(aConf, ""));
var tC = allText(outC);
...
var qConf = xfContentQuality(fakeConf, dir, "recap", mC.confirmedFacts, xfActualActivityData(aConf, ""));
...
var qReg = xfContentQuality(fakeReg, dir, "recap", mR.confirmedFacts, xfActualActivityData(aReg, ""));
check((qReg.flags || []).indexOf("unsupported:实际人数") < 0,
  "质量检查：\"报名 10 人\"(非到场)不被误判为人数虚构", "flags=" + (qReg.flags || []).join(","));

// --- 7) actualActivityData 默认不确认天气/完成/人数（避免回退到虚构）---
var adDef = xfActualActivityData(mkAct({}), "");
