...
// ② 归一化 JSON 经 visionToPhotoMeta 映射成 photoMeta 形态，再经 piApplyVisionNormalized 写回缓存
// ③ 写回后 photoMeta 标记为真实（simulated=false），piAnalyzeOne 自动采用真实字段（业务代码零改动）
...
const okWrite = (typeof piApplyVisionNormalized === "function") ? piApplyVisionNormalized("vtest.jpg", norm) : false;
chk("P2-4-标准化写回入口可用", okWrite === true);
chk("P2-4-映射函数存在", typeof visionToPhotoMeta === "function" && typeof piApplyVisionNormalized === "function");
...
const an = (typeof piAnalyzeOne === "function") ? piAnalyzeOne("vtest.jpg", 0, null) : null;
...
// 非法/缺字段不写回（健壮性）
const bad = (typeof piApplyVisionNormalized === "function") ? piApplyVisionNormalized("bad.jpg", null) : "fn-missing";
